...
d = {'dealer_card':[8, 10, 11], 'init_hand':[15, 12, 19], 'p_bust_odds':[.6, .2, .8], 'hit':[0,0,0], 'card_count':[0, 0, 0]}   
input_df = pd.DataFrame(data=d)
...
action = ['stay', 'hit']
print('Invidual hand selections:')
print(action[model_decision(new_model, 15, 9, 0.5, 0)])
print(action[model_decision(new_model, 17, 8, 0.7, 0)])
print(action[model_decision(new_model, 16, 10, 0.7, 0)])
print(action[model_decision(new_model, 11, 10, 0, 0)])
